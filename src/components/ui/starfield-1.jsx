...
const Starfield = ({
  ...
  const reset = () => { stop(); sd.current.star.arr = []; init(); };
  ...
  const clickHandler = event => {
    if (event.type === 'mousedown') setState(prev => ({ ...prev, hyperspace: true }));
    if (event.type === 'mouseup') setState(prev => ({ ...prev, hyperspace: false }));
  };
  ...
  useEffect(() => {
    if (state.reset) { reset(); setState(prev => ({ ...prev, reset: false })); }
    if (state.stop) { stop(); setState(prev => ({ ...prev, stop: false })); }
    if (state.start) { init(); setState(prev => ({ ...prev, start: false })); }
